...
void write_student() //local
{
	student st;
	ofstream outFile; // ofstream for write
	outFile.open("student.txt",ios::binary|ios::app); //ios::app (appending)
	st.getdata();
...
void delete_student(int n)
{
	...
	while(inFile.read(reinterpret_cast<char *> (&st), sizeof(student)))
	{
		if(st.retrollno() != n)
		{
			outFile.write(reinterpret_cast<char *> (&st), sizeof(student)); //NULL
		}
	}
	outFile.close();
	inFile.close();
